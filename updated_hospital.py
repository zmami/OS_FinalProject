    ...
    def record_visit(self, day, patient):
        with self.lock:
            ...
            if patient.doctor_start_time and patient.arrival_time:
                # Convert seconds to minutes and multiply by 600 to make waiting times more realistic
                # This will show waiting times in the range of 60-120 minutes for regular patients
                wait_time = (patient.doctor_start_time - patient.arrival_time) * 600 / 60
...
class HospitalSimulation:
    ...
    def simulate_time(self, seconds):
        ...
        scaled_time = min(scaled_time, 0.5)  # Increased from 0.1 to make simulation even slower
    ...
    def nurse_assessment_thread(self):
        ...
        while not self.simulation_complete.is_set():
            try:
                ...
                # Simulate assessment time (increased to 30-60 minutes)
    ...
    def surgery_thread(self):
        ...
        while not self.simulation_complete.is_set():
            try:
                ...
                # Standard death chance is 25% (increased from 10%)
                death_chance = 0.25
                
                # Increased death chance during MCI
                if self.is_mci_day and self.mci_in_progress:
                    # 40% chance of death during MCI (increased from 20%)
                    death_chance = 0.40
                    
                    # Even higher chance for MCI patients
                    if patient.is_mci_patient:
                        death_chance = 0.50  # 50% chance of death for MCI patients (increased from 25%)
                ...
                if random.random() < death_chance:
                    ...
                    # Nurse checks on patient as required in specs
        ...
        while not self.simulation_complete.is_set():
            try:
                ...
                with self.code_blue_lock:
                    ...
                    # Acquire 2 ER doctors and 1 nurse as specified in the requirements
                    for _ in range(2):
                    ...
                    # Determine outcome (20% chance to save the patient as specified)
                    if random.random() < 0.20:  # 20% survival rate (increased from 30%)
    ...
    def ambulance_thread(self):
        ...
        while not self.simulation_complete.is_set():
            try:
                ...
                # Try to acquire an ER doctor and nurse as specified in requirements
                ...
                while self.mci_in_progress and not self.simulation_complete.is_set():
                    try:
                        ...
                        if surgery_needed:
                            ...
                            if random.random() < 0.30:  # 30% chance of death without surgery (increased from 15%)
                                patient.dead = True
    ...
    def regular_doctor_thread(self, department):
        ...
        while not self.simulation_complete.is_set():
            try:
                ...
                # Simulate doctor examination time (increased to 20-40 minutes)
                examination_time = random.uniform(20, 40)
                self.simulate_time(examination_time)
                
                print(f"👨‍⚕️ ({self.format_time()}) Doctor in {department} examined {patient.name}")
                
                # Check if surgery is needed (30% chance as specified in requirements)
    ...
    def er_doctor_thread(self, queue_idx):
        ...
        while not self.simulation_complete.is_set():
            try:
                ...
                # Check for Code Blue event (15% chance for ER patients - increased from 5%)
                ...
                if patient.is_mci_patient:
                    surgery_needed = random.random() < 0.50  # 50% chance for MCI patients
                else:
                    surgery_needed = random.random() < 0.30  # 30% chance as required
                ...
                if surgery_needed:
                    ...
                    if patient.is_mci_patient and random.random() < 0.30:  # 30% chance (increased from 15%)
                        patient.dead = True
                        print(f"💀 ({self.format_time()}) MCI patient {patient.name} died during treatment")
                    else:
    ...
    def generate_regular_patients(self, day):
        ...
        interval = 5  # Significantly shorter for faster simulation
    ...
    def generate_ambulance_arrivals(self, day):
        ...
        interval = 15  # Significantly shorter for faster simulation
    ...
    def simulate_day(self, day):
        ...
        max_wait_time = 30  # Increased from 5 to 30 seconds to wait for queue processing
    ...
    def run_simulation(self):
        ...
        timeout = 300  # Increased from 60 to 300 seconds (5 minutes) for the whole simulation
...
def main():
    ...
    simulation = HospitalSimulation(
        ...
        simulation_speed=100.0    # Significantly reduced from 100.0 to make simulation much slower
    )
